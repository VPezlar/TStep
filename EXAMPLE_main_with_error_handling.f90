PROGRAM main
    ...
    USE error_handling  ! NEW: Import error handling module
    ...
    IF (error_status /= 0) THEN
        CALL log_error(ERR_MAIN_CONFIG)  ! NEW: Use centralized error logging
    ...
    IF (STATUS_CODE /= 0) THEN
        CALL log_error(ERR_MAIN_EXT_CMD)  ! NEW: Use centralized error logging
    ...
    IF (error_status /= 0) THEN
        CALL log_error(ERR_MAIN_READ_FLOW)  ! NEW: Use centralized error logging
    ...
    IF (error_status /= 0) THEN
        CALL log_error(ERR_MAIN_DISTURBANCE)  ! NEW: Use centralized error logging
        CALL cleanup_allocations()
    ...
    IF (error_status /= 0) THEN
        CALL log_error(ERR_MAIN_WRITE_OUTPUT)  ! NEW: Use centralized error logging
        CALL cleanup_allocations()
