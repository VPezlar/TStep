PROGRAM main
    ...
    ! 
    ! Test matrix: n=2000, eigenvalues = [1, 2, 3, ..., 2000]
    ! Construction: A = Q @ diag(eigvals) @ Q^T (symmetric, well-conditioned)
    ! Expected: Arnoldi recovers largest eigenvalues close to 2000
    ! ===================================================================
    ...
    ALLOCATE(v_normalized(2000), STAT=error_status)
    IF (error_status /= 0) THEN
    ...
    DO i = 1, MIN(5, SIZE(eigenvalues))
        WRITE(*,'(I3,2X,F12.6,2X,F12.6,2X,F12.6,2X,I5)') i, &
            REAL(eigenvalues(i)), AIMAG(eigenvalues(i)), ABS(eigenvalues(i)), 2001 - i
    END DO
    WRITE(*,*) '-------------------------------------------------------'
...
CONTAINS
    ...
    SUBROUTINE validate_eigenvalues()
        ! Validates test matrix eigenvalues: n=2000, m=200
        ! Matrix A has eigenvalues 1, 2, 3, ..., 2000
        ! With large Krylov subspace, top 5 should be very close to 2000, 1999, 1998, 1997, 1996
        ...
        DO k = 1, MIN(5, m_size)
            ...
            expected_val = REAL(2001 - k, rk)  ! 2000, 1999, 1998, 1997, 1996
